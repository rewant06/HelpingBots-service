...
export function PostCard({ post, onDelete }: PostCardProps) {
  ...
  const requireAuth = (intent: "react" | "vote", force = false) => {
    if (user && !force) return true;

    toast("Login required", {
      description:
        intent === "react"
          ? "Please login to react to posts."
          : "Please login to vote in polls.",
      action: {
        label: "Login",
        onClick: () =>
  ...
  const { setReaction, setVoted, initializeInteraction } =
    useInteractionStore();
  ...
  useEffect(() => {
    initializeInteraction(post.id, {
      ...
      votedOptionId: post.votedOptionId ?? null,
    });
  }, [
    post.id,
    post.userReaction,
  ...
  const votedOptionId =
    interaction?.votedOptionId ?? post.votedOptionId ?? null;
  // --- 4. LOCAL UI STATE ---
  ...
  const handleVote = async (optionId: string) => {
    ...
    const prevOptions = pollOptions.map((o) => ({ ...o }));

    // Optimistic Update
    setVoted(post.id, optionId);
    setPollOptions((prev) =>
      prev.map((o) =>
    ...
    try {
      ...
      initializeInteraction(post.id, {
        reaction: interaction?.reaction ?? post.userReaction ?? null,
        hasVoted: false,
        votedOptionId: null,
      });
  ...
  const handleReact = async (type: "AGREE" | "DISAGREE") => {
    ...
    if (currentReaction === type) {
      ...
      if (currentReaction === "DISAGREE")
        setDisagreeCount((p) => Math.max(0, p - 1));
    ...
    try {
      ...
      setAgreeCount(prev.agreeCount);
      setDisagreeCount(prev.disagreeCount);
      setReaction(post.id, prev.reaction);

      const status = e?.response?.status;
      if (status === 401 || status === 403) {
        requireAuth("react", true);
        return;
      }
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      if (response && response.triggered) {
        ...
        toast.error("Safety protocol triggered");
      } else {
            ...
            {blockedWords.length > 0 && (
              <Alert variant="destructive" className="py-2">
                ...
                <AlertDescription className="text-xs">
                  Restricted:{" "}
                  <span className="font-bold">{blockedWords.join(", ")}</span>
                </AlertDescription>
              </Alert>
            )}
            <Textarea
            ...
            {pollOptions.map((option) => {
              ...
              return (
                <button
                  ...
                  aria-label={
                    isMine ? `${option.text} (your vote)` : option.text
                  }
                  key={option.id}
                  className={cn(
                    "relative w-full text-left overflow-hidden rounded-md border h-9 sm:h-10 transition-all select-none group",
                    hasVoted
                      ? "cursor-not-allowed border-border/40 opacity-90"
                      : "cursor-pointer hover:border-primary/40 active:scale-[0.99] active:bg-muted/30",
                    isMine && "border-primary/60 ring-2 ring-primary/20",
                  )}
                ...
                >
                  ...
                  {hasVoted && (
                    <div
                      className={cn(
                        "absolute inset-0 transition-all duration-500 ease-out origin-left",
                        isMine ? "bg-primary/15" : "bg-primary/10",
                      )}
                      style={{ width: `${percent}%` }}
                    />
                  )}
          ...
          <div className="flex gap-1.5">
            ...
            >
              <ThumbsUp
                className={cn(
                  "w-3.5 h-3.5",
                  currentReaction === "AGREE" && "fill-current",
                )}
              />
            ...
            >
              <ThumbsDown
                className={cn(
                  "w-3.5 h-3.5 mt-0.5",
                  currentReaction === "DISAGREE" && "fill-current",
                )}
              />
              <span className="text-xs font-bold tabular-nums">
                {disagreeCount}
